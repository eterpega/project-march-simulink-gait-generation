...
%% Angular velocity
subplot(4,2,3)
time2=[time,repmat(time(end),1,length(time))+time];
velocityHip_RPM2=[velocityHip_RPM;velocityHip_RPM]';
velocityKnee_RPM2=[velocityKnee_RPM;velocityKnee_RPM]';
plot(time2,velocityHip_RPM2,'Linewidth',2);
hold on
plot(time2,velocityKnee_RPM2,'Linewidth',2);
title('Hip and Knee Angular Velocity')
xlabel('Time [s]')
ylabel('Velocity [RPM]')
legend('angleHip','angleKnee')
hold on
plot(time2,repmat(rads2RPM(velocityMaximumHip) ,2*length(time),1),'Linewidth',2,'Color','r')
hold on
plot(time2,repmat(rads2RPM(-velocityMaximumHip),2*length(time),1),'Linewidth',2,'Color','r')
grid on
