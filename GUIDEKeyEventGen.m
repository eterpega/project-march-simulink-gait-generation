...
if strcmpi('Yes',choice)
    ...
    itemList{8} = 'Rough Terrain Right';
    itemList{9} = 'Rough Terrain Left';
    itemList{10} = 'Manual';
    ...
    [s,v] = listdlg('PromptString','Select the type of gait',...
     ...
     if strcmpi(gaitType,'Continuous') && ~(s~=1|s~=10)
     ...
     %save sitDown vectors to MD
        ...
        %roughTerrainRight
        elseif s==8 && v==1
        write_to_model_dictionary(myDictionaryObj, sectionObj, 'roughTerrainStepRightKnee', 'roughTerrainStepRightHip', gait);
        
        %roughTerrainLeft
        elseif s==9 && v==1
      	write_to_model_dictionary(myDictionaryObj, sectionObj, 'roughTerrainStepLeftKnee', 'roughTerrainStepLeftHip', gait);
...
= compute_splines(handles, selected);
maxVelocityKneeRPM=max(abs(angleKnee_RPM))
maxVelocityHipRPM=max(abs(angleHip_RPM))
